...
#copied from https://github.com/mononitogoswami/tsad-model-selection/blob/master/src/tsadams/model_selection/inject_anomalies.py
...
class InjectAnomalies:
    ...
    def compute_window_size(self, T):
        ...
        if len(self.peaks) == 0:
            ...
            #when there is just one peak, we can't compute the window size, so we set it to empty 
            raise AnomalyOnePeakError('Only one peak found in the autocorrelation. Cannot compute window size. Try to increase time series generation length to obtain more peaks.')
        else:
            window_size = min(
